...
class HuaRunGasFlowHandler(config_entries.ConfigFlow):
    """华润燃气配置流程处理器"""
    VERSION = 1
    ...
    async def async_step_user(self, user_input=None):
        ...
        # 初始化i18n
        if not self.i18n:
            self.i18n = HuarunI18n(self.hass, DOMAIN)
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Required(
                    CONF_CNO,
                    description={"name": self.i18n.get_text("step.user.data.attr_cno", "用户编号")}
                ): str,
                vol.Optional(
                    CONF_NAME,
                    default=DEFAULT_NAME,
                    description={"name": self.i18n.get_text("step.user.data.attr_name", "传感器名称")}
                ): str,
                vol.Optional(
                    CONF_UPDATE_INTERVAL,
                    default=str(DEFAULT_UPDATE_INTERVAL),
                    description={
                        "name": self.i18n.get_text("step.user.data.attr_update_interval", "数据更新间隔（小时）"),
                        "description": f"({MIN_UPDATE_INTERVAL}-{MAX_UPDATE_INTERVAL}小时)"
                    }
                ): str,
            }),
            errors={k: self.i18n.get_text(f"errors.{v}", v) for k, v in errors.items()},
        )
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    def __init__(self, config_entry: ConfigEntry):
        """初始化 - 不要设置self.config_entry，基类已经提供了"""
        super().__init__()
        # self.config_entry 是基类的属性，不需要重新设置
        self.i18n = None
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Required(
                    ...
                    description={"name": self.i18n.get_text("step.options.data.attr_cno", "用户编号")}
                ): str,
                vol.Optional(
                    CONF_NAME,
                    default=current_data.get(CONF_NAME, DEFAULT_NAME),
                    description={"name": self.i18n.get_text("step.options.data.attr_name", "传感器名称")}
                ): str,
                vol.Optional(
                    CONF_UPDATE_INTERVAL,
                    default=str(current_data[CONF_UPDATE_INTERVAL]),
                    description={
                        "name": self.i18n.get_text("step.options.data.attr_update_interval", "数据更新间隔（小时）"),
                        "description": f"({MIN_UPDATE_INTERVAL}-{MAX_UPDATE_INTERVAL}小时)"
                    }
                ): str,
            }),
